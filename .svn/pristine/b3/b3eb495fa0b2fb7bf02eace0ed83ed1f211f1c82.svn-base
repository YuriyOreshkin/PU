...
namespace PU
{
    ...
    public class TableChanges : IDisposable
    {
        ...
        static TableChanges()
        {
            ...
            _appmutex = new Mutex(false, "Global\\" + Classes.Utils.NormalizeName(_appid.ToString()));
        }
            ...
            string leftdb, string rightdb)
        {
            ...
            if (leftCount + rightCount < CACHE_BUFFER_SIZE)
            {
                ...
                sb.PageSize = 4096;
                _main = new SQLiteConnection(sb.ConnectionString, true);
                _main.Open();
        ...
        public void SetColumnField(string diff, long itemRowIndex, string columnName, bool right, object updatedValue)
        {
            ...
            using (SQLiteConnection dbconn = new SQLiteConnection(sb.ConnectionString, true))
            {
                dbconn.Open();
                ...
                try
                {
                    ...
                    if (Classes.Utils.IsColumnActingAsRowIdAlias(table, columnName))
                    {
                    ...
                    if (right)
                    {
                        ...
                        if (Classes.Utils.GetColumnByName(_leftTable.Columns, columnName) != null &&
                    ...
                    else
                    {
                        ...
                        if (Classes.Utils.GetColumnByName(_rightTable, columnName) != null && 
                            (citem.Result == ComparisonResult.Same || citem.Result == ComparisonResult.DifferentData))
                        {
        ...
        public void DeleteRows(string diff, List<TableChangesRange> rows, bool left, RowsDeletedProgressHandler handler)
        {
            ...
            using (SQLiteConnection to = Classes.Utils.MakeDbConnection(left ? _leftdb : _rightdb, true))
            {
                to.Open();
        ...
        public long SearchRowsBySQL(bool isLeft, string diff, long rowIndex, long maxRowIndex, string sql, SearchRowsProgressHandler handler)
        {
            ...
            using (SQLiteConnection conn = Classes.Utils.MakeDbConnection(dbpath, false))
            {
                conn.Open();
        ...
        public void CopyRows(string diff, List<TableChangesRange> rows, bool leftToRight, RowsCopyingProgressHandler handler)
        {
            ...
            List<SQLiteColumnStatement> common = Classes.Utils.GetCommonColumns(
                leftToRight?_rightTable:_leftTable, leftToRight?_leftTable:_rightTable, false, out dcols);

            // Open the database to copy rows from
            using (SQLiteConnection from = Classes.Utils.MakeDbConnection(leftToRight ? _leftdb : _rightdb, false))
            {
                from.Open();
                ...
                try
                {
                    ...
                    using (SQLiteConnection to = Classes.Utils.MakeDbConnection(leftToRight ? _rightdb : _leftdb, true))
                    {
                        to.Open();
...
//                            _log.Error("error occured while cleaning table changes database", ex);
        ...
        private List<string> GetCommonColumnNames()
        {
            ...
            List<SQLiteColumnStatement> common = Classes.Utils.GetCommonColumns(_leftTable, _rightTable);
        ...
        private void ValidateRowsCopyOperation(bool leftToRight)
        {
            ...
            // table.
            List<SQLiteColumnStatement> pkeys = Classes.Utils.GetPrimaryColumns(targetTable);
            foreach (SQLiteColumnStatement pk in pkeys)
            {
                if (!Classes.Utils.ColumnListContains(common, pk))
                {
            ...
            {
                if (!Classes.Utils.ColumnListContains(common, col))
                {
            ...
            SQLiteTransaction fromtx, ref SQLiteTransaction totx, RowsCopyingProgressHandler handler)
        {
            ...
            SQLiteCommand insert = new SQLiteCommand("INSERT INTO " + _tableName.ToString() + " (" +
                Classes.Utils.BuildColumnsString(common, false) + ") VALUES (" +
                Classes.Utils.BuildColumnParametersString(common) + "); select last_insert_rowid()",
                totx.Connection, totx);
            Classes.Utils.AddCommandColumnParameters(insert, common);
            ...
            bool leftToRight, RowsCopyingProgressHandler handler)
        {
            ...
            for (int i = 0; i < common.Count; i++)
            {
                ...
                string prmName = Classes.Utils.GetColumnParameterName(colName);
                sb.Append(colName + " = " + prmName);
                DbType dsttype = Classes.Utils.GetDbType(common[i].ColumnType);
                update.Parameters.Add(prmName, dsttype);

                SQLiteColumnStatement origCol = null;
                if (dcols != null)
                    origCol = Classes.Utils.GetColumnByName(dcols, colName);
                DbType srctype = dsttype;
                if (origCol != null)
                    srctype = Classes.Utils.GetDbType(origCol.ColumnType);
                ...
                Classes.Utils.AssignParameterValue(update.Parameters[prmName], value);
            ...
            List<SQLiteColumnStatement> common, bool leftToRight, RowsCopyingProgressHandler handler)
        {
            ...
            for (int i = 0; i < common.Count; i++)
            {
                string pname = Classes.Utils.GetColumnParameterName(common[i].ObjectName.ToString());
                ...
                if (Classes.Utils.GetDbType(common[i].ColumnType) == DbType.Binary)
                {
                    // Since the actual value that gets stored in the table change item is either 0 (when the BLOB field is null)
                    // or 1 (when the BLOB field is not null) - we'll have to translate this when inserting the row to the
                    // target table.
        ...
        {
            SQLiteColumnStatement column = Classes.Utils.FindColumn(table.Columns, columnName);
            ...
            SQLiteCommand res = new SQLiteCommand(
                ...
                dbconn, tx);
            DbType dbtype = Classes.Utils.GetDbTypeFromClrType(column, updatedValue);
        ...
        private SQLiteCommand PrepareReadRowCommand(SQLiteCreateTableStatement table, SQLiteConnection conn)
        {
            ...
            string clist = Classes.Utils.BuildColumnsString(table.Columns, true);
            SQLiteCommand res = new SQLiteCommand(
        ...
        private SQLiteConnection CreateTableChangesDB(string tableName)
        {
            ...
            SQLiteConnection main = new SQLiteConnection(sb.ConnectionString, true);
            main.Open();
        ...
        private string MakeTempDbName(string tableName)
        {
            ...
            if (utc < 0)
                utc = utc * -1;
            tableName = Classes.Utils.NormalizeName(tableName+"_G_"+_appid.ToString()+"_G_");
            do
            {    
