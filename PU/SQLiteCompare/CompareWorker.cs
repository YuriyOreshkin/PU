...
namespace PU
{
    ...
    public class CompareWorker : IWorker
    {
        ...
        {
            List<SQLiteColumnStatement> res = Classes.Utils.GetPrimaryColumns(table);
            if (res.Count > 0)
                return true;
            else
                return false;
        ...
        private Dictionary<SchemaObject, Dictionary<string, SQLiteDdlStatement>> ParseDB(string fpath)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(sb.ConnectionString, true))
            {
                conn.Open();
                ...
                using (SQLiteDataReader reader = query.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        sql = Classes.Utils.StripComments(sql);
