...
namespace PU
{
    ...
    public class TableCompareWorker : IWorker
    {
            ...
            SQLiteCreateTableStatement rightTable, string leftdb, string rightdb)
        {
            ...
            string errmsg;
            if (!Classes.Utils.IsTableComparisonAllowed(leftTable, rightTable, out errmsg, _compareBlobs))
                throw new ApplicationException(errmsg);

            // Get the list of common columns
            List<SQLiteColumnStatement> common = Classes.Utils.GetCommonColumns(rightTable, leftTable);

            // Get the list of primary key columns
            List<SQLiteColumnStatement> pkeys = Classes.Utils.GetPrimaryColumns(leftTable);
            ...
            try
            {
                ...
                if (Classes.Utils.ContainsBlobColumn(pkeys) || (pkeys.Count==0 && Classes.Utils.ContainsBlobColumn(common)))
                {
            ...
            string leftstr, string rightstr, TableChanges res)
        {
            ...
            using (SQLiteConnection right = new SQLiteConnection(rightstr, true))
            {
                right.Open();

                using (SQLiteConnection left = new SQLiteConnection(leftstr, true))
                {
                    left.Open();
            ...
            using (SQLiteConnection left = new SQLiteConnection(leftstr, true))
            {
                ...
                using (SQLiteConnection right = new SQLiteConnection(rightstr, true))
                {
                    ...
                    try
                    {
                        ...
                        List<SQLiteColumnStatement> common = Classes.Utils.GetCommonColumns(leftTable, rightTable, false, out dcols);

                        // Used for optimizing search
                        bool hasBlobColumn = Classes.Utils.ContainsBlobColumn(common);
                        ...
                        string clist = Classes.Utils.BuildColumnsString(common, false);
                    ...
                    while (rightReader.Read())
                    {
                        ...
                        if (haspkey)
                        {
                            ...
                            foreach (SQLiteColumnStatement col in common)
                            {
                                ...
                                if (!Classes.Utils.ObjectsAreEqual(lval, rval))
                                {
                ...
                using (SQLiteConnection left = new SQLiteConnection(leftstr, true))
                {
                    ...
                    try
                    {
                        ...
                        SQLiteCommand select = new SQLiteCommand(
                            @"SELECT RowID," + Classes.Utils.BuildColumnsString(common, false) + " FROM " + tblname, right, rightTx);
            ...
            using (SQLiteConnection left = new SQLiteConnection(leftstr, true))
            {
                ...
                using (SQLiteConnection right = new SQLiteConnection(rightstr, true))
                {
                    ...
                    try
                    {
                        ...
                        // This command will be used to iterate over the entire table
                        // in the left database.
                        List<SQLiteColumnStatement> leftCommon = Classes.Utils.GetMatchingColumns(_leftTable.Columns, common);
                        ...
                        bool hasBlobColumns = Classes.Utils.ContainsBlobColumn(leftCommon);
        ...
        {
            string clist = Classes.Utils.BuildColumnsString(common, true);
            SQLiteCommand select = new SQLiteCommand(@"SELECT RowID, "+clist+" FROM " +
                tableName, conn, tx);
            return select;
        }
                ...
                while (rightReader.Read())
                {
                    ...
                    if (haspkey)
                    {
                        ...
                        foreach (SQLiteColumnStatement col in common)
                        {
                            ...
                            SQLiteColumnStatement leftCol = Classes.Utils.GetColumnByName(_leftTable, colname);
                            SQLiteColumnStatement rightCol = Classes.Utils.GetColumnByName(_rightTable, colname);
                            if (Classes.Utils.IsColumnTypesMatching(leftCol, rightCol))
                            {
                                if (Classes.Utils.GetDbType(col.ColumnType) == System.Data.DbType.Binary)
                                {
                            ...
                            {
                                DbType ltype = Classes.Utils.GetDbType(leftCol.ColumnType);
                                DbType rtype = Classes.Utils.GetDbType(rightCol.ColumnType);
            ...
            bool castBlobs)
        {
            ...
            {
                string clist = Classes.Utils.BuildColumnsString(common, castBlobs);
            ...
            for(int i=0; i<pkeys.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (Classes.Utils.GetDbType(col.ColumnType) == DbType.DateTime)
                        sb.Append("DATETIME(" + colname + ") = DATETIME(" + prmname+")");
                    else
                        sb.Append(colname + " = " + prmname);

                    DbType dbtype = Classes.Utils.GetDbTypeFromClrType(value);
