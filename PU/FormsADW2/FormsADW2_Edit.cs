...
{
    public partial class FormsADW2_Edit : Telerik.WinControls.UI.RadForm
    {
        pu6Entities db = new pu6Entities();
        public string action;
        public FormsADW_2 adw_2 { get; set; }
        public long StaffID { get; set; }
        private bool cleanData = true;

        public FormsADW2_Edit()
        {
            InitializeComponent();
        }

        public static FormsADW2_Edit SelfRef
        {
            get;
            set;
        }
        ...
        private void StaffAnketaEdit_Load(object sender, EventArgs e)
        {
            ...
            if (action == "edit")
            {
                adw_2 = db.FormsADW_2.FirstOrDefault(x => x.ID == adw_2.ID);

                LastName_.Text = adw_2.LastName;
                FirstName_.Text = adw_2.FirstName;
                MiddleName_.Text = adw_2.MiddleName;
                numSpinEditor.Value = adw_2.Num.HasValue ? adw_2.Num.Value : 1;
                MiddleNameCancel.Checked = !String.IsNullOrEmpty(adw_2.MiddleNameCancel);
                PlaceBirthCancel.Checked = !String.IsNullOrEmpty(adw_2.PlaceBirthCancel);

                if (adw_2.DateBirth != null && adw_2.DateBirth.HasValue)
                    DateBirth_.Value = adw_2.DateBirth.Value;

                if (adw_2.Sex != null && adw_2.Sex.HasValue)
                {
                    if (adw_2.Sex.Value == 0)
                ...
                if (adw_2.Doc_Type_ID.HasValue)
                {
                    ConfirmDocType.Items.FirstOrDefault(x => x.Value.ToString() == adw_2.Doc_Type_ID.Value.ToString()).Selected = true;
                    ConfirmDocName.Text = adw_2.Doc_Name;
                    ConfirmDocSerLat.Text = adw_2.Ser_Lat;
                    ConfirmDocSerRus.Text = adw_2.Ser_Rus;
                    ConfirmDocNum.Text = adw_2.Doc_Num;
                    if (adw_2.Doc_Date.HasValue)
                        ConfirmDocDate.Value = adw_2.Doc_Date.Value;
                    ConfirmDocKemVyd.Text = adw_2.Doc_Kem_Vyd;
                }

                if (adw_2.Type_DateBirth == 1)
                {
                    DateOs_CheckBox.Checked = true;
                    DayOs_SpinEditor.Value = adw_2.DateBirthDay_Os.HasValue ? adw_2.DateBirthDay_Os.Value : 0;
                    MonthOs_SpinEditor.Value = adw_2.DateBirthMonth_Os.HasValue ? adw_2.DateBirthMonth_Os.Value : 0;
                    YearOs_SpinEditor.Value = adw_2.DateBirthYear_Os.HasValue ? adw_2.DateBirthYear_Os.Value : 0;
                }

                Phone.Text = adw_2.Phone;
                if (adw_2.DateFilling.HasValue)
                    DateUnderwrite.Value = adw_2.DateFilling.Value;
                else
                    DateUnderwrite.Value = DateTime.Now;


                if (adw_2.Type_PlaceBirth.HasValue)
                {
                    Type_PlaceBirth.Items.FirstOrDefault(x => x.Tag.ToString() == adw_2.Type_PlaceBirth.Value.ToString()).Selected = true;
                }

                Punkt.Text = adw_2.Punkt;
                Distr.Text = adw_2.Distr;
                Region.Text = adw_2.Region;
                Country.Text = adw_2.Country;
                Citizenship.Text = adw_2.Citizenship;

                Reg_Addr.Text = adw_2.Reg_Addr;
                Fakt_Addr.Text = adw_2.Fakt_Addr;

            }
            else
            {
                adw_2 = new FormsADW_2();
                DateUnderwrite.Value = DateTime.Now;
//                radPageView1.Pages["radPageViewPage2"].Enabled = false;
        ...
        private void radButton3_Click(object sender, EventArgs e)
        {
            ...
            switch (action)
            {
                case "add":
                    try
                    {
                        //if (db.FormsADW_2.Any(x => x.StaffID == StaffID && x.Num == adw_2.Num))
                        //{
                        //    RadMessageBox.Show(this, "Дублирование записи.\r\nФорма АДВ-2 с таким номером уже есть в базе данных!", "Внимание!", MessageBoxButtons.OK, RadMessageIcon.Exclamation, MessageBoxDefaultButton.Button1);
                        //    return;
                        //}
                ...
                case "edit":
                    try
                    {
                        if (db.FormsADW_2.Any(x => x.StaffID == StaffID && x.ID != adw_2.ID && x.Num == adw_2.Num))
                        {
                            RadMessageBox.Show(this, "Дублирование записи.\r\nФорма АДВ-2 с таким номером уже есть в базе данных!", "Внимание!", MessageBoxButtons.OK, RadMessageIcon.Exclamation, MessageBoxDefaultButton.Button1);
                            return;
                        }
        ...
        {
            adw_2.FirstName = FirstName_.Text;
            adw_2.StaffID = StaffID;
            adw_2.LastName = LastName_.Text;
            adw_2.MiddleName = MiddleName_.Text;
            adw_2.MiddleNameCancel = MiddleNameCancel.Checked ? "ОТМН" : "";
            adw_2.Num = (short)numSpinEditor.Value;

            if (SexMRadioButton.IsChecked || SexFRadioButton.IsChecked)
            {
                adw_2.Sex = SexMRadioButton.IsChecked ? (byte)0 : (byte)1;
            }


            if (!String.IsNullOrEmpty(DateBirth_.Text) && !DateOs_CheckBox.Checked)
            {
                adw_2.DateBirth = DateBirth_.Value;
            }
            else
            {
                adw_2.DateBirth = null;
            }

            adw_2.Type_DateBirth = DateOs_CheckBox.Checked ? (short)1 : (short)0;
            if (DateOs_CheckBox.Checked)
            {
                adw_2.DateBirthDay_Os = (short)DayOs_SpinEditor.Value;
                adw_2.DateBirthMonth_Os = (short)MonthOs_SpinEditor.Value;
                adw_2.DateBirthYear_Os = (short)YearOs_SpinEditor.Value;
            }

            adw_2.Phone = Phone.Text;
            adw_2.DateFilling = DateUnderwrite.Value;
            adw_2.DateUpdate = DateTime.Now;

            adw_2.Type_PlaceBirth = short.Parse(Type_PlaceBirth.SelectedItem.Tag.ToString());
            adw_2.Punkt = Punkt.Text;
            adw_2.Distr = Distr.Text;
            adw_2.Region = Region.Text;
            adw_2.Country = Country.Text;
            adw_2.Citizenship = Citizenship.Text;

            adw_2.PlaceBirthCancel = PlaceBirthCancel.Checked ? "ОТМН" : "";

            adw_2.Fakt_Addr = Fakt_Addr.Text;
            adw_2.Reg_Addr = Reg_Addr.Text;

            if (ConfirmDocType.Text == "")
                adw_2.Doc_Type_ID = null;
            else
            {
                adw_2.Doc_Type_ID = long.Parse(ConfirmDocType.SelectedItem.Value.ToString());
            }


            adw_2.Doc_Name = ConfirmDocName.Text;
            adw_2.Ser_Lat = ConfirmDocSerLat.Text;
            adw_2.Ser_Rus = ConfirmDocSerRus.Text;
            adw_2.Doc_Num = ConfirmDocNum.Text;

            if (ConfirmDocDate.Text == "")
                adw_2.Doc_Date = null;
            else
                adw_2.Doc_Date = ConfirmDocDate.Value.Date;
            adw_2.Doc_Kem_Vyd = ConfirmDocKemVyd.Text;
        }
